...
function MemberCenter() {
  ...
  // 取得資料 (使用 useCallback 避免重新渲染時產生新函式)
  ...
  return (
    <>
      ...
      {/* 載入中遮罩 */}
      {isLoading && (
        <div className="position-fixed top-0 start-0 w-100 h-100 d-flex justify-content-center align-items-center bg-white bg-opacity-75 z-3">
            ...
            <div className="col-md-4">
              <div className="bg-white rounded-4 shadow-sm p-24">
                ...
                <div className="nav flex-column nav-pills gap-2">
                  {[
                    { id: 'user', label: '個人資訊' },
                    { id: 'messages', label: '通知訊息' },
                    { id: 'orders', label: '訂單資訊' },
                    { id: 'follow', label: '我的關注' }
                  ].map((tab) => (
                    <button 
                      key={tab.id}
                      className={`nav-link text-start p-16 rounded-3 ${activeTab === tab.id ? 'active' : 'text-secondary'}`}
                  ...
                  <hr />
                  <button className="nav-link text-start p-16 text-danger border-0 bg-transparent" onClick={handleLogout}>登出</button>
                </div>
              </div>
            </div>
                    ...
                    <form>
                      ...
                      <div className="mb-16">
                        ...
                        <div className="d-flex align-items-center gap-3">
                          ...
                          <button type="button" className="btn btn-link text-danger fs-14 p-0 text-decoration-none">修改密碼</button>
                        </div>
                      </div>
                      <div className="mb-16">
                        ...
                        {displayFavorites.map(product => (
                          <div className="col-6 col-lg-4" key={product.id}>
                            <div className="card h-100 rounded-4 overflow-hidden border-0 shadow-sm position-relative shadow-hover">
                              <button 
                                className="position-absolute top-0 end-0 m-2 btn btn-light btn-sm rounded-circle shadow-sm z-2"
